...
const REAL_SLUG = 'jewish-ghetto-food-tour';
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `https://${SITE_DOMAIN}/tours/jewish-ghetto-tour` },
  };
}

export default async function JewishGhettoTourPage() {
  const tour = await getTourBySlug(REAL_SLUG);
  if (!tour) notFound();

  const others = await Promise.all(
    KEPT_TOURS.filter((t) => t.path !== 'jewish-ghetto-tour').map(async (t) => {
